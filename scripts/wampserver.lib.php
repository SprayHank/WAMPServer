...
function wampIniSet($iniFile, $params) {
	...
	foreach($params as $param => $value) $iniFileContents = preg_replace('|' . $param . ' = .*|', $param . ' = ' . '"' . $value . '"', $iniFileContents);
	$fp = fopen($iniFile, 'w');
	...
	if($handle = opendir($dir)) {
		while(FALSE !== ($file = readdir($handle))) {
			if($file != "." && $file != ".." && is_dir($dir . '/' . $file)) {
				...
				if(!isset($result) || $result == 1) $list[] = $file;
			}
		}
		closedir($handle);
	}
	if(isset($list)) return ($list); else
		return (NULL);
}
...
function checkPhpConf($baseDir, $version) {
	...
	if(!is_file($baseDir . '/' . $version . '/' . $wampBinConfFiles)) return (0);
	if(!is_file($baseDir . '/' . $version . '/' . $phpConfFileForApache)) return (0);
...
function switchPhpVersion($newPhpVersion) {
	...
	foreach($httpdContents as $line) {
		...
		} elseif($phpConf['apache'][$apacheVersion]['AddModule'] != '' && strstr($line, 'AddModule') && strstr($line, 'php')) $newHttpdContents .= 'AddModule ' . $phpConf['apache'][$apacheVersion]['AddModule'] . "\r\n"; else
