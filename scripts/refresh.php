...
foreach($langList as $langname => $langstatus) {
	...
	if($langList[$langname] == 1) $langText .= 'Type: item; Caption: "' . $cleanLangName . '"; Glyph: 13; Action: multi; Actions: lang_' . $cleanLangName . '
'; else
...
foreach($ext as $extname => $extstatus) {
	if($ext[$extname] == 1) $extText .= 'Type: item; Caption: "' . $extname . '"; Glyph: 13; Action: multi; Actions: php_ext_' . $extname . '
'; else
...
foreach($ext as $extname => $extstatus) {
	if($ext[$extname] == 1) $extText .= '[php_ext_' . $extname . ']
...
foreach($params_for_wampini as $paramname => $paramstatus) {
	if($params_for_wampini[$paramname] == 1) $phpConfText .= 'Type: item; Caption: "' . $paramname . '"; Glyph: 13; Action: multi; Actions: ' . $phpParams[$paramname] . '
'; else
...
foreach($params_for_wampini as $paramname => $paramstatus) {
	if($params_for_wampini[$paramname] == 1) $phpConfText .= '[' . $phpParams[$paramname] . ']
Action: service; Service: WAMPApache; ServiceAction: stop; Flags: waituntilterminated
...
foreach($mod as $modname => $modstatus) {
	if($mod[$modname] == 1) $httpdText .= 'Type: item; Caption: "' . $modname . '"; Glyph: 13; Action: multi; Actions: apache_mod_' . $modname . '
'; else
...
foreach($mod as $modname => $modstatus) {
	if($mod[$modname] == 1) $httpdText .= '[apache_mod_' . $modname . ']
Action: service; Service: WAMPApache; ServiceAction: stop; Flags: waituntilterminated
...
foreach($aliasDirContents as $one_alias) {
	...
	if(isset($match[1])) $newalias_dest = $match[1]; else $newalias_dest = NULL;
...
foreach($phpVersionList as $onePhp) {
	...
	if($onePhpVersion === $wampConf['phpVersion']) $phpGlyph = '; Glyph: 13';
...
foreach($apacheVersionList as $oneApache) {
	...
	if(isset($apacheConf)) $apacheConf = NULL;
	include $c_apacheVersionDir . '/apache' . $oneApacheVersion . '/' . $wampBinConfFiles;

	if($oneApacheVersion === $wampConf['apacheVersion']) $apacheGlyph = '; Glyph: 13';
...
foreach($mysqlVersionList as $oneMysql) {
	...
	if(isset($mysqlConf)) $mysqlConf = NULL;
	include $c_mysqlVersionDir . '/mysql' . $oneMysqlVersion . '/' . $wampBinConfFiles;
	if($oneMysqlVersion === $wampConf['mysqlVersion']) $myreplace .= 'Type: item; Caption: "' . $oneMysqlVersion . '"; Action: multi; Actions:switchMysql' . $oneMysqlVersion . '; Glyph: 13
'; else
